...
# Get the absolute path to the scripts directory and the project root
SCRIPTS_DIR = os.path.dirname(os.path.abspath(__file__))
WORK_EFFORTS_MODULE_DIR = os.path.dirname(SCRIPTS_DIR)
PROJECT_ROOT = os.path.dirname(WORK_EFFORTS_MODULE_DIR)

# Define the package installation paths
PKG_AI_SETUP_DIR = os.path.join(PROJECT_ROOT, ".AI-Setup")
PKG_WORK_EFFORTS_DIR = os.path.join(PKG_AI_SETUP_DIR, "work_efforts")
PKG_TEMPLATE_PATH = os.path.join(PKG_WORK_EFFORTS_DIR, "templates", "work-effort-template.md")
PKG_ACTIVE_PATH = os.path.join(PKG_WORK_EFFORTS_DIR, "active")

# We'll set these dynamically based on whether we're using current directory or not
WORK_EFFORTS_DIR = PKG_WORK_EFFORTS_DIR
TEMPLATE_PATH = PKG_TEMPLATE_PATH
ACTIVE_PATH = PKG_ACTIVE_PATH
...
def get_active_directory(use_current_dir=False):
    ...
    Args:
        use_current_dir: If True, use the .AI-Setup directory in the current working directory
    ...
    if use_current_dir:
        ...
        # Create a .AI-Setup/work_efforts/active directory in the current directory if it doesn't exist
    ...
    # When using package paths, ensure the directories exist
    if not os.path.exists(PKG_AI_SETUP_DIR):
        os.makedirs(PKG_AI_SETUP_DIR, exist_ok=True)
    if not os.path.exists(PKG_WORK_EFFORTS_DIR):
        os.makedirs(PKG_WORK_EFFORTS_DIR, exist_ok=True)
    if not os.path.exists(PKG_ACTIVE_PATH):
        os.makedirs(PKG_ACTIVE_PATH, exist_ok=True)

    return PKG_ACTIVE_PATH

def get_template_path(use_current_dir=False):
    """Get the path to the template file, creating it if necessary.

    Args:
        use_current_dir: If True, use a template in the current directory's .AI-Setup folder
    ...
    if use_current_dir:
        current_dir = os.getcwd()
        ai_setup_dir = os.path.join(current_dir, ".AI-Setup")
        templates_dir = os.path.join(ai_setup_dir, "work_efforts", "templates")
        local_template_path = os.path.join(templates_dir, "work-effort-template.md")

        # Create directories if they don't exist
        os.makedirs(templates_dir, exist_ok=True)

        # If the template doesn't exist in the current directory, create it with default content
        if not os.path.exists(local_template_path):
            # Try to copy from package template if it exists
            pkg_template_exists = os.path.exists(PKG_TEMPLATE_PATH)

            if pkg_template_exists:
                try:
                    with open(PKG_TEMPLATE_PATH, "r") as src_template:
                    ...
                    create_default_template(local_template_path)
            else:
    ...
    # When using package paths, ensure the template directory exists
    pkg_templates_dir = os.path.join(PKG_WORK_EFFORTS_DIR, "templates")
    if not os.path.exists(pkg_templates_dir):
        os.makedirs(pkg_templates_dir, exist_ok=True)

    # Create the template if it doesn't exist
    if not os.path.exists(PKG_TEMPLATE_PATH):
        create_default_template(PKG_TEMPLATE_PATH)

    return PKG_TEMPLATE_PATH

def create_default_template(template_path):
    """Create a default work effort template file.
...
def parse_arguments():
    ...
    parser.add_argument(
        "--title",
        default="Untitled",
        help="Title of the work effort (default: Untitled)"
    )
    parser.add_argument(
        "--assignee",
        default="self",
        help="Assignee of the work effort (default: self)"
    )
    parser.add_argument(
        "--priority",
        default="medium",
    ...
    parser.add_argument(
        "--description",
        help="Description for AI content generation (required if --use-ai is specified)"
    )
    parser.add_argument(
        "--model",
        default="phi3",
        help="Ollama model to use for content generation (default: phi3)"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        "--use-current-dir",
        action="store_true",
        help="Create work effort in current directory instead of package directory"
    )
    parser.add_argument(
        "--package-dir",
        action="store_true",
        help="Create work effort in the package directory (default) - overrides --use-current-dir"
    )
...
async def main_async():
    ...
    # Check if .AI-Setup directory exists and create it if it doesn't
    current_dir = os.getcwd()
    ai_setup_dir = os.path.join(current_dir, ".AI-Setup")
    if not os.path.exists(ai_setup_dir) and args.use_current_dir:
        print(f"ℹ️ .AI-Setup directory not found. Creating it at: {ai_setup_dir}")
        os.makedirs(ai_setup_dir, exist_ok=True)
        work_efforts_dir = os.path.join(ai_setup_dir, "work_efforts")
        os.makedirs(work_efforts_dir, exist_ok=True)
        os.makedirs(os.path.join(work_efforts_dir, "active"), exist_ok=True)
        os.makedirs(os.path.join(work_efforts_dir, "templates"), exist_ok=True)
        os.makedirs(os.path.join(work_efforts_dir, "completed"), exist_ok=True)
        os.makedirs(os.path.join(work_efforts_dir, "archived"), exist_ok=True)
    ...
    if not args.use_current_dir:
        # Ensure the package directories exist
        if not os.path.exists(PKG_AI_SETUP_DIR):
            print(f"ℹ️ Package .AI-Setup directory not found. Creating it at: {PKG_AI_SETUP_DIR}")
            os.makedirs(PKG_AI_SETUP_DIR, exist_ok=True)
        if not os.path.exists(PKG_WORK_EFFORTS_DIR):
            os.makedirs(PKG_WORK_EFFORTS_DIR, exist_ok=True)
            os.makedirs(os.path.join(PKG_WORK_EFFORTS_DIR, "active"), exist_ok=True)
            os.makedirs(os.path.join(PKG_WORK_EFFORTS_DIR, "templates"), exist_ok=True)
            os.makedirs(os.path.join(PKG_WORK_EFFORTS_DIR, "completed"), exist_ok=True)
            os.makedirs(os.path.join(PKG_WORK_EFFORTS_DIR, "archived"), exist_ok=True)
    ...
    if args.interactive:
        ...
        # Determine where to create the work effort
        if args.package_dir:
            # Use package directory explicitly
            use_current_dir = False
            print("Creating in package directory")
        elif args.use_current_dir:
            # Use current directory explicitly
            use_current_dir = True
            print("Creating in current working directory")
        else:
            # Logic for determining default behavior
            # If no arguments were provided (using defaults), create in current directory by default
