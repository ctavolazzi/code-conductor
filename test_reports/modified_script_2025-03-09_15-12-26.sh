...
# Test 1: Initialize a project with yes | code-conductor setup
print_header "Test 1: Initialize project with yes | code-conductor setup"
echo "Running yes | code-conductor setup in $TEST_DIR"
...
echo "Running yes | code-conductor setup in $SEPARATE_DIR"
yes | code-conductor setup
...
cd "$TEST_DIR"
yes | code-conductor work --title "Root Location Test" --assignee "Tester" --priority "high"
