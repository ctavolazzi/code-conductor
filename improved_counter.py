#!/usr/bin/env python3
"""
Improved Work Effort Counter module for maintaining persistent work effort numbering.
...
class WorkEffortCounter:
    ...
    def __init__(self, counter_file_path: Optional[str] = None, logger=None):
        """
...
def initialize_counter_from_existing_work_efforts(work_efforts_dir: str, counter_file: Optional[str] = None) -> int:
    ...
    # Updated regex patterns to handle both 4-digit and larger numbers
    # Pattern for standard numbered work efforts (e.g., "0001_", "10000_")
    ...
    def check_directory(directory):
        ...
        for item in os.listdir(directory):
            if os.path.isdir(os.path.join(directory, item)) or item.endswith(".md"):
                # For directories or markdown files, check for standard pattern
                match = re.match(standard_pattern, item)
                if match:
                    try:
                        number = int(match.group(1))
                        highest_number = max(highest_number, number)
                        logging.info(f"Found numbered item: {item} with number {number}")
                    except ValueError:
                        pass
    ...
    logging.info(f"Counter initialized to {next_number} based on existing work efforts")
...
def format_work_effort_filename(title: str, count: int, use_date_prefix: bool = False) -> str:
    ...
    sanitized_title = title.lower().replace(" ", "_").replace("/", "_").replace("\\", "_")

    # Combine number and title
    return f"{formatted_count}_{sanitized_title}"